<template>
  <view class="container">
    ...
    <view class="form">
      ...
      <button @tap="handleSubmit" class="submit-btn">保存</button>
    </view>
  </view>
</template>

<script>
import { updateCategory } from '@/api/category.js'
...
export default {
  ...
  methods: {
    async handleSubmit() {
      ...
      try {
        const res = await updateCategory(this.categoryId, this.formData)
        if (res.code === 1) {
          uni.showToast({
            title: '更新成功',
            icon: 'success'
